...
describe("formatLocalDate", () => {
  // 用本地时间构造，避免期望值随运行环境时区偏移
  const date = new Date(2025, 0, 5, 15, 42);
  // 运行时默认 locale 的兜底输出（fallback 行为的对照基准）
  const runtimeLongDate = new Intl.DateTimeFormat(undefined, {
    dateStyle: "long",
  }).format(date);
  ...
  it("lets Intl fall back for structurally valid but unsupported locales", () => {
    ...
    expect(formatLocalDate(date, "PPP", "zz-ZZ")).toBe(runtimeLongDate);
  });

  it("falls back to the runtime locale for malformed locale tags", () => {
    expect(() => formatLocalDate(date, "PPP", "not_a_locale")).not.toThrow();
    expect(formatLocalDate(date, "PPP", "not_a_locale")).toBe(runtimeLongDate);
  });
});
