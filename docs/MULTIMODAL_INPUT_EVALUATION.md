...
1. **ASR 供应商默认值**：建议默认文档给 SiliconFlow（免费 + 中文优），代码不写死，纯配置切换——是否接受？
2. **纯附件发送**：v1 要求附件必须伴随文字指令（`content.min(1)`），是否放宽为允许"纯语音/纯文件"发送？
3. **附件 pending 时点发送**：拒绝发送（UI 等待 ready）还是自动等待后发送？建议 v1 拒绝并提示（实现最简）。
4. **音频书签是否参与向量嵌入**（语义搜索）？参与则 P1 需加 embeddings 分支（成本低，建议参与）。
5. **Chat 附件与书签资产共用存储配额**？建议共用（实现为默认行为，无需决策）。
6. **Mobile 系统分享菜单直接收音频**是否纳入本期？建议 P2 之后。
