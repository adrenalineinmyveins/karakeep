...
Saiye supports distributed tracing and structured event logging via OpenTelemetry. When enabled, traces and logs are collected for tRPC API calls, background worker operations, and other key workflows. Saiye also exports prometheus-based metrics.
...
| OTEL_SERVICE_NAME                  | No       | saiye | The service name that will appear in your tracing backend. The actual service name will include a suffix (e.g., `saiye-api`, `saiye-workers`).                                                                                                                                                                    |
